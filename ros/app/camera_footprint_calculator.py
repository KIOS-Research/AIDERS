...
class CameraFootprintCalculator:
    """
    example:
    ...
    @staticmethod
    def getBoundingPolygon(latitude, longitude, FOVh, FOVv, altitude, roll, pitch, heading):
        ...
        ray11 = CameraFootprintCalculator.ray1(FOVh, FOVv)
        ray22 = CameraFootprintCalculator.ray2(FOVh, FOVv)
        ray33 = CameraFootprintCalculator.ray3(FOVh, FOVv)
        ray44 = CameraFootprintCalculator.ray4(FOVh, FOVv)

        rotatedVectors = CameraFootprintCalculator.rotateRays(
                ray11, ray22, ray33, ray44, roll, pitch, heading)
        
        origin = Vector(0, 0, altitude)
        intersections = CameraFootprintCalculator.getRayGroundIntersections(rotatedVectors, origin)

        polygon = []
        for intersection in enumerate(intersections):
            # print(intersection[0])
            # meters to degrees
            shift_latitude = intersection[1].x / 111139
            shift_longitude = intersection[1].y / (111139 * math.cos(math.radians(latitude)))
            # new coordinates
            new_latitude = latitude + shift_latitude
            new_longitude = longitude + shift_longitude
            polygon.append((new_latitude, new_longitude))
    ...
    def getRayGroundIntersections(rays, origin):
        ...
        #     intersections[i] = CameraFootprintCalculator.findRayGroundIntersection(rays[i], origin);
        # }
        ...
        for i in range(len(rays)):
            intersections.append( CameraFootprintCalculator.findRayGroundIntersection(rays[i], origin) )
        return intersections
