...
{
// match anything which looks like a decimal degrees coordinate pair
    let isLatLng = query.match(
        /^[ ]*(?:Lat: )?(-?\d+\.?\d*)[, ]+(?:Lng: )?(-?\d+\.?\d*)[ ]*$/i
    );

    let drone = is_query_contains_drone_id(query);
    let loraDevce = getLoraDeviceIfExists(query)
    if (isLatLng)
    {
...
//             geocodes.push(coordinateFeature(lat, lng));
    ...
    if (loraDevce !== -1)
    {
        ...
        let lng = Number(loraDevce["data"]["coordinates"]["lon"]);
        let lat = Number(loraDevce["data"]["coordinates"]["lat"]);
        let tagName = loraDevce["data"]["tagName"]
        geocodes = [];
        ...
        {
// must be lat, lng
            geocodes.push(loraCoordinateFeature(lng, lat,tagName));
        }
