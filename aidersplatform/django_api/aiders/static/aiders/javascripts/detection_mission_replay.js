{
        ...
        setInterval(function () {
            ...
            for (let i = 0; i < allDronesArr.length; i++) {
                if (
                    ...
                    webSocketMessage['drones'] !== undefined
                ) {
                    if (webSocketMessage.drones[allDronesArr[i].droneID] !== undefined) {
                        objectDetected = webSocketMessage['drones'][allDronesArr[i].droneID]['detection_object'];
                        if (objectDetected !== undefined) {
                            visualize_detected_objects(
                                webSocketMessage['drones'][allDronesArr[i].droneID]['detection_object'],
                                allDronesArr[i].droneID
                            );
                        }
                    }
                }
                if (allDronesArr[i].droneDetectionStatus === DETECTION_CONNECTED && !allDronesArr[i].detVideoStarted) {
                    ...
                    allDronesArr[i].droneDetectionStatus === DETECTION_DISCONNECTED &&
                    allDronesArr[i].detVideoStarted
                ) {
                    clear_detection_timeout();
                            ...
                            get_drone_detection_state(droneID).then(function (detection_drone_state) {
                                if (detection_drone_state !== DETECTION_CONNECTED) {
                                    ...
                                    let msg =
                                        'Timeout error: Drone ' +
                                        droneID +
                                        ' failed to start detection after ' +
                                        timeoutMs / 1000 +
                                        ' seconds';
                                    showPopupForALittle('#detection_fail_box', msg, 4000);
                                    update_drone_detection_state_on_api(
                                        allDrones[droneIndex].droneID,
                                        DETECTION_WANT_TO_DISCONNECT
                                    );
                                }
                            });
                        },
     ...
     * */
    function visualize_detected_objects(detected_cars, drone) {
        ...
        webSocketMessage['drones'][drone]['detection_object'] = [];
    }
    ...
    function get_drone_detection_state(droneID) {
        ...
        webSocketMessage['drones'].forEach(function (drone) {
            if (drone['drone_name'] == droneID) {
                returnObjectStatus = drone['detection'];
            }
        });
    ...
    function update_drone_detection_state_on_api(droneID, detection_status, detectorType = 'None') {
        ...
        if (detectorType === 'None') {
            data = {
                detection: { drone_name: droneID, operation_name: CURRENT_OP, detection_status: detection_status },
            };
        } else {
            data = {
                detection: {
    ...
    function start_detection_vid(droneID) {
        ...
        function change() {
            if (webSocketMessage['drones'][droneID] !== undefined) {
                latest_frame_path = webSocketMessage['drones'][droneID]['detection_frame']['frame'];
            } else {
                latest_frame_path = NO_ACTIVE_DETECTION_SESSION_ERROR_MESSAGE;
            }
            let currentDrone = get_drone_object(droneID);
            let status_div_id = document.getElementById('status-div-' + droneID);

            if (latest_frame_path === NO_ACTIVE_DETECTION_SESSION_ERROR_MESSAGE) {
            ...
            if (latest_frame_path === NO_ACTIVE_DETECTION_SESSION_ERROR_MESSAGE) {
                ...
                document.getElementById('det-img-' + droneID).src = dutils.urls.resolve('detection_frame_path', {
