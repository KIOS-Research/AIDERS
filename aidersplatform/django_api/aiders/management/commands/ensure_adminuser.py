...
and can be used in the following way using Bash: 
 ...
 python manage.py ensure_adminuser --username=${USERNAME} \
--email=${EMAIL} \
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--superUserExists',help='Checks if the specified username exists in the database', action='store_true')
        parser.add_argument('--createSuperuser',help='Creates a superuser account with the specified details, if it does not already exist', action='store_true')
    def handle(self, *args, **options):
        User = get_user_model()
        if options['superUserExists']:
            superUsers = User.objects.filter(is_superuser=True)
            print(True) if superUsers else print(False)
            # print (True) if User.objects.filter(username=options['username']).exists() else print (False)
