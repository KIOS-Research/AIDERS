...
class TestViews(TestSetUp):
    ...
    def test_BuildMapGetLastImageAPIView_view_post_fail(self):
        self.client.force_login(self.user)
        operation_instance = OperationFactory.create(
            operation_name=self.operation_name, operator=self.user)
        drone_instance = DroneFactory.create(
            drone_name=self.drone_name, operation=operation_instance)
        response = self.client.post(reverse('get_last_build_map_image', args=[
                                    self.operation_name]), {'drone_id': self.drone_name})
        self.assertEquals(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_BuildMapGetLastImageAPIView_view_post_success(self):
        self.client.force_login(self.user)
        operation_instance = OperationFactory.create(
            operation_name=self.operation_name, operator=self.user)
        drone_instance = DroneFactory.create(
            drone_name=self.drone_name, operation=operation_instance)
        image_instance = BuildMapImageFactory1.create_batch(1)
        session_instance = BuildMapSessionFactory.create(
            user=self.user, operation=operation_instance, drone=drone_instance)
        session_instance.images.set(image_instance)
        response = self.client.post(reverse('get_last_build_map_image', args=[
                                    self.operation_name]), {'drone_id': self.drone_name})
        self.assertEquals(response.status_code, status.HTTP_202_ACCEPTED)

    def test_BuildMapGetLastImageAPIView_view_deny_anonymous(self):
        OperationFactory.create(
            operation_name=self.operation_name, operator=self.user)
        response = self.client.get(reverse('get_last_build_map_image', args=[
                                   self.operation_name]), follow=True)
        self.assertRedirects(response, reverse(
            'login')+'?next='+reverse('get_last_build_map_image', args=[self.operation_name]))

    def test_BuildMapGetLastAPIView_view_post_success(self):
        self.client.force_login(self.user)
        operation_instance = OperationFactory.create(
            operation_name=self.operation_name, operator=self.user)
        drone_instance = DroneFactory.create(
        ...
        session_instance = BuildMapSessionFactory.create(
            user=self.user, operation=operation_instance, drone=drone_instance)
        response = self.client.post(reverse('get_last_build_map', args=[
                                    self.operation_name]), {'drone_id': self.drone_name})
        self.assertEquals(response.status_code, status.HTTP_202_ACCEPTED)

    def test_BuildMapGetLastAPIView_view_post_fail(self):
        self.client.force_login(self.user)
        operation_instance = OperationFactory.create(
            operation_name=self.operation_name, operator=self.user)
        drone_instance = DroneFactory.create(
    ...
    def test_BuildMapGetLastAPIView_view_deny_anonymous(self):
        operation_instance = OperationFactory.create(
            operation_name=self.operation_name, operator=self.user)
        drone_instance = DroneFactory.create(
            drone_name=self.drone_name, operation=operation_instance)
        response = self.client.post(reverse('get_last_build_map', args=[
                                    self.operation_name]), {'drone_id': self.drone_name}, follow=True)
        self.assertRedirects(response, reverse(
            'login')+'?next='+reverse('get_last_build_map', args=[self.operation_name]))
    ...
    """WeatherLiveAPIView, WeatherStationAPIView"""
