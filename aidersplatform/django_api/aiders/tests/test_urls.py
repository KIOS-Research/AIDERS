from aiders.views import (AlgorithmListView, AlgorithmRetrieveView,
                          BuildMapGetLastAPIView, BuildMapGetLastImageAPIView,
                          BuildMapImageView, BuildMapLoadAPIView,
                          BuildMapAPIView, ControlDeviceDataAPIView,
                          ControlDeviceMonitoringView,
                          ControlDevicesMonitoringView, DatabaseFiller,
                          DetectionRetrieveAPIView, DroneList,
                          DroneListCreateAPIView, DroneModifyOperationView,
                          DroneRetrieveAPIView, EditOperationForm,
                          ExecuteAlgorithmAPIView, ExecuteMissionAPIView,
                          FirePredictionCreateAPIView, ManageOperationsView,
                          ManagePermissionsView, MissionListCreateAPIView,
                          MissionLoggerListCreateAPIView,
                          MissionPointsListCreateAPIView,
                          MissionRetrieveAPIView, NewOperationForm,
                          OperationListCreateAPIView,
                          ReplayMissionOnlineAPIView, SystemMonitoringView,
                          TelemetryListCreateAPIView, TelemetryRetrieveAPIView,
                          UserDetail, UserList, WeatherLiveAPIView,
                          WeatherStationAPIView, buildMapSessionsAPIView,
                          buildMapSessionsShareAPIView,
                          detection_types_api_view, frontEndUserInputAPIView,
                          index, join_operation_view, leave_operation_view,
                          login_view, logout_view, register_request,
                          stop_operation_view)
...
class TestWebAppUrls(SimpleTestCase):
    ...
    def test_new_operation_url_resolves(self):
        url = reverse('new_operation')
        self.assertEquals(resolve(url).func.view_class,NewOperationForm)

    def test_edit_operation_url_resolves(self):
        url = reverse('edit_operation', args=['some_operation_name'])
        self.assertEquals(resolve(url).func.view_class,EditOperationForm)
    ...
    def test_start_build_map_url_resolves(self):
        ...
        self.assertEquals(resolve(url).func.view_class, BuildMapAPIView)

    def test_get_last_build_map_image_url_resolves(self):
        url = reverse('get_last_build_map_image', args=['some_operation_name'])
        self.assertEquals(resolve(url).func.view_class,BuildMapGetLastImageAPIView)

    def test_get_last_build_map_url_resolves(self):
        url = reverse('get_last_build_map', args=['some_operation_name'])
        self.assertEquals(resolve(url).func.view_class,BuildMapGetLastAPIView)
    ...
    def test_weather_live_url_resolves(self):
        url = reverse('weather_live', args=['some_operation_name'])
        self.assertEquals(resolve(url).func.view_class,WeatherLiveAPIView)
