from inference.interfaces import Outcome, Node, BayesianNetworkOutput
...
if TYPE_CHECKING:
    ...
    from inference.interfaces import BayesianNetworkData
...
class BayesianNetworkInference:
    ...
    def bn_inferences(self, bayesian_network_data: List["BayesianNetworkData"],
        ...
        inference_objects = list(filter(lambda x: bool(x.bayesian_network_id), bayesian_network_data))

        if len(inference_objects) >= 1:
            bn_outputs = [self._worker_inference(inference_objects[0])]
            # bn_outputs = multiprocessing_pool.map(self._worker_inference, inference_objects)
            # when checking for issues in the inference process:
            # for inference_object in inference_objects:
            #     bn_outputs.append(self._worker_inference(inference_object))
    ...
    def _bn_inference(self, specific_bn_data: "BayesianNetworkData") -> BayesianNetworkOutput:
        ...
        pedestrian_id = specific_bn_data.actor_id
        ...
        output_network = self._inference_query_and_build_infered_bn_output(bn_instance, output_nodes, evidence_query,
                                                                           pedestrian_id, bn_id, all_nodes_with_outcomes)
    ...
    def _add_ids_to_output(bn_network_output: BayesianNetworkOutput, specific_bn_data: "BayesianNetworkData"
        ...
        """
        bn_network_output.actor_id = specific_bn_data.actor_id
