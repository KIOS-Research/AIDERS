...
class ObjectDetector:
    ...
    def process_frame(self, frame, sess, max_outputs):
        ...
        return frame, box_center_points, labels
    ...
    def run_inference(self, img_path, output_img_path):
        ...
        out_frame, box_center_points, labels = self.process_frame(
        ...
        for p in box_center_points:
            ...
            if labels[i] == "boat":
                database.queries.saveDetectedObject(lat, lon, labels[i], random.randint(1,10), 1.2, self.operationId, self.droneId, self.sessionId, frameId)
            i += 1
    ...
    def start_loop(self):
        ...
        while not self.stop:
            ...
            if currentTime >= nextDetectionTime:
                ...
                if latestLiveStreamFrame is None:
                    print(f"{self.droneName}: No live stream frame available, waiting for available frame.")
                    time.sleep(1)
                    continue
