...
class VideoIO:
    ...
    def __init__(self, size, input_uri,
        ...
        self.cap_fps = self.source.get(cv2.CAP_PROP_FPS) if 15 < self.source.get(cv2.CAP_PROP_FPS)<=30 else 15
    ...
    def _gst_cap_pipeline(self):
        ...
        elif self.protocol == Protocol.RTMP:
            print("rtmp test")
            pipeline = 'rtmpsrc location=%s ! decodebin ! ' % self.input_uri
            # return ' videoscale ! queue ! x264enc pass=pass1 threads=1 bitrate=5000 tune=zerolatency ' + \
            #  ' ! queue ! flvmux streamable=true ! rtmpsink location=' + self.input_uri +' live=1 max-lateness=100 '+ \
            #  ' ! video/x-raw, width='+ str(self.resolution[0]) +' ,height='+str(self.resolution[1])+' ! appsink'

            # pipeline = 'rtmpsink location=%s  ! ' % self.input_uri
    ...
    @classmethod
    def get(cls, drone_id):
        print("ASKING FOR OBJECT WITH INPUT URI: ", drone_id)
        for inst in cls.instances:
            print("CURRENT INSTANCE INPUT URI: ", inst.drone_id)
            if inst.drone_id == drone_id:
