...
def nms(tlbrs, scores, nms_thresh):
# def nms(tlwhs, scores, nms_thresh):
    ...
    # print('tlbr',tlbrs)
    # print('scores',scores)
    areas = (tlbrs[:, 2] - tlbrs[:, 0] + 1) * (tlbrs[:, 3] - tlbrs[:, 1] + 1)
...
@nb.njit(fastmath=True, cache=True)
def nms_old(tlwhs, scores, nms_thresh):
    """Applies Non-Maximum Suppression on the bounding boxes [x, y, w, h].
    Returns an array with the indexes of the bounding boxes we want to keep.
    """
    areas = tlwhs[:, 2] * tlwhs[:, 3]
...
@nb.njit(cache=True, inline='always')
def get_center_wh(tlbr):
    xywh = np.empty(4)
    xywh[0] = (tlbr[0] + tlbr[2]) / 2

    xywh[1] = tlbr[1] + tlbr[3]
    xywh[2:] = get_size(tlbr)
