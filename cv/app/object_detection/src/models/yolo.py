...
class YOLOv4Tiny3l(YOLO):
    ENGINE_PATH = Path(__file__).parent / 'tiny_yolov4_3l_custom_best.trt'  #'yolov4-tiny.trt'
    MODEL_PATH = Path(__file__).parent / 'tiny_yolov4_3l_custom_best.onnx'#'tyv4_3l_30000.onnx' #'yolov4-tiny.onnx'
    NUM_CLASSES = 4
    INPUT_SHAPE = (3, 512, 512)
    LAYER_FACTORS = [32, 16, 8]
    SCALES = [1.05, 1.05,1.05]
    ANCHORS = [[39, 37,  27, 65,  58, 93],
               [14, 14,  20, 25,  15, 42],
	       [9,  9,   6, 13,   9, 23]]


class YOLOv4Tiny3l_tile(YOLO):
    ENGINE_PATH = Path(__file__).parent / 'tyv4_3l_b8.trt'  #'yolov4-tiny.trt'
    MODEL_PATH = Path(__file__).parent / 'tyv4_3l_b8.onnx'#'tyv4_3l_30000.onnx' #'yolov4-tiny.onnx'
    NUM_CLASSES = 4
    INPUT_SHAPE = (3, 512, 512)
    LAYER_FACTORS = [32, 16, 8]
    SCALES = [1.05, 1.05,1.05]
    ANCHORS = [[ 26, 85,  50, 47,  63, 92],
               [25, 23,  15, 50,  25, 34],
	            [10, 10,   7, 16,  16, 18]]
    BATCHES = 8


class YOLOv4Tiny_tile(YOLO):
    ENGINE_PATH = Path(__file__).parent / 'tyv4_ppl_veh_512_low.trt'  #'yolov4-tiny.trt'
    MODEL_PATH = Path(__file__).parent / 'tyv4_ppl_veh_512_low.onnx'#'tyv4_3l_30000.onnx' #'yolov4-tiny.onnx'
    NUM_CLASSES = 5
    INPUT_SHAPE = (3, 512, 512)
    LAYER_FACTORS = [32, 16]
    SCALES = [1.05, 1.05]
    ANCHORS = [[ 55, 29, 45, 49, 91, 70],
               [ 11, 10, 22, 22, 31, 46]]
    BATCHES = 16


class YOLOv4Veh(YOLO):
    ENGINE_PATH = Path(__file__).parent / 'vehicles_yv4_smbigobj_30000.trt'
    MODEL_PATH = Path(__file__).parent /  'vehicles_yv4_smbigobj_30000.onnx'
    NUM_CLASSES = 4
    INPUT_SHAPE = (3, 608, 608)
    LAYER_FACTORS = [8, 16, 32]
    SCALES = [1.2, 1.1, 1.05]
    ANCHORS = [[10, 10,   7, 16,  16, 18],
               [25, 23,  15, 50,  25, 34],
               [ 26, 85,  50, 47,  63, 92]]
