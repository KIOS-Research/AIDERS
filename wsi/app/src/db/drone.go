...
// fetches a drone's operation id from the database
func GetDroneOperationId(droneId int64) (int, error) {
	// Assuming Conn is your database connection
	query := "SELECT operation_id FROM aiders_drone WHERE id = ?"
	var operationId int
	err := Conn.QueryRow(query, droneId).Scan(&operationId)
	if err != nil {
		if err == sql.ErrNoRows {
			return 0, nil // No operation found for this droneId
		}
...
func UpdateDroneConnectionStatusAndType(_droneID int64, _status int, _type string, _connectionType string) error {
	...
	log.Printf("%c Updating drone '%s' connection status to %d", '\U0001F4BB', _type, _status)
	var query string
	if _status == 0 {
...
func SaveDrone(droneObj map[string]interface{}) int64 {
	...
	res, err := stmt.Exec(
		...
		droneObj["multispectralAvailable"], droneObj["lidarAvailable"], 0, 0, droneObj["type"], droneObj["connection_type"], droneObj["configuration"],
	)
	if err != nil {
		return 0
	}
...
func SaveDroneTelemetry(droneId int64, secondsOn int, telemetryMsg DroneTelemetryMessage, missionLogId int, operationId int, fovPolygon [][2]float64) (telemetryId int64, err error) {
	...
	res, err := Conn.Exec(query, droneId, telemetryMsg.Latitude, telemetryMsg.Longitude, telemetryMsg.Altitude, telemetryMsg.Heading, telemetryMsg.Velocity,
		telemetryMsg.GpsSignal, telemetryMsg.SatelliteNumber, telemetryMsg.HomeLatitude, telemetryMsg.HomeLongitude, telemetryMsg.DroneState, nil,
...
// saves an incoming pilot notification to the database
func SaveIncomingPilotNotification(_name, _msg string, _droneOperationId int) error {
	query := "INSERT INTO aiders_pilotnotification (sender, receiver, incoming, message, operation_id, timestamp) VALUES (?, ?, ?, ?, ?, ?)"
	_, err := Conn.Exec(query, _name, "platform", 1, _msg, _droneOperationId, time.Now())
	if err != nil {
		return fmt.Errorf("error saving pilot notification: %w", err)
	}
	return nil
}