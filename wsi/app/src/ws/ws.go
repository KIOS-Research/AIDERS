...
func clientConnected(_ws *websocket.Conn, _name string, _droneModel string, _cameraModel string) {
	...
	if droneId != 0 {
		if isAlreadyConnected {
			...
			db.UpdateDroneConnectionStatusAndType(droneId, 1, "DJI", "WEBSOCKETS") // flag drone as connected in the database
...
func clientDisconnected(_name string, _err error) {
	...
	if droneId != 0 {
		db.UpdateDroneConnectionStatusAndType(droneId, 0, "DJI", "WEBSOCKETS") // flag drone as disconnected in the database
	...
	for {
		...
		go func() {
			...
			if tempMsg.Telemetry != nil {
				...
				middleware.SendTelemetryToKafka(_name, droneId, tempMsg.Telemetry, droneMsg)
		...
		// Handle incoming notification from the mobile app in a goroutine
		go func() {
			defer wg.Done() // Decrement the WaitGroup counter when done
			if tempMsg.Notification != nil {
				...
				log.Printf("Incoming Notification Message from Pilot: %+v", notification)
				
				// save notification to the database
				droneOperationId, _ := db.GetDroneOperationId(droneId)
				db.SaveIncomingPilotNotification(_name, notification.NotificationMsg, droneOperationId)
			}
		}()
