...
class BuildMap {
    ...
    async update() {
        ...
        postGetActiveBuildMapSessionImages(postData).then((_response) => {
...
function updateBuildMapSessionOnLoadedBuildMapSessionList(loadedBuildMapSessionObject) {
    let existingIndex = loadedBuildMapSession.findIndex(
        (obj) => obj.sessionId === loadedBuildMapSessionObject.sessionId,
    );
...
async function startOrStopBuildMapRequest(_droneName, _command) {
    // TODO: Multispectral update change start_multispectral_build_map
    let sessionId;
    let data = {
        drone_name: _droneName,
        start_build_map_boolean: _command,
        interval: document.getElementById("buildMapInterval").innerHTML,
        start_multispectral_build_map: false,
    };
    postStartOrStopBuildMapRequest(data).then((_response) => {
        sessionId = _response.data;
    });
    return sessionId;
}
...
function getImagesByUsingSessionId(_session) {
    ...
    postGetImagesByUsingSessionId(data).then((_response) => {
        let buildMapImages = _response.data;
        buildMapImages.forEach((image) => {
...
async function checkIfDroneBuildMapIsActivatedOrDeactivated(_drone, _activate) {
    if (_activate && liveBuildMap[_drone.droneID] === undefined) {
        ...
        let activatedSessionId = await startOrStopBuildMapRequest(_drone.droneID, true);
...
function startDroneBuildMap() {
    ...
    selectedDrones.forEach(async function (drone) {
        ...
        activatedSessionId = await startOrStopBuildMapRequest(drone.droneID, true);
...
function stopDroneBuildMap() {
    ...
    selectedDrones.forEach(function (drone) {
        ...
        startOrStopBuildMapRequest(drone.droneID, false);
...
function loadDroneBuildMap() {
    getBuildMapSessions().then((_response) => {
        let availableBuildMapSessionsNotLoadedYet = _response.filter(
            (obj2) => !loadedBuildMapSession.some((obj1) => obj1.sessionId == obj2.sessionId),
        );

        createPopupDialogForLoadClearProcess(
            availableBuildMapSessionsNotLoadedYet,
            "Select Build Map Sessions to Load",
...
function clearDroneBuildMap() {
    ...
    createPopupDialogForLoadClearProcess(loadedBuildMapSession, "Select Build Map Sessions to Clear").then(
