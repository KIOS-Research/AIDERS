...

{
    ...
    function create_new_drone_model(index) {
        let { droneID } = allDroneInfo[index];
...
function createDroneObjectOnMap(drone_list) {
    new_drone_names = [];
    drone_names_and_ids = [];
    webSocketMessage['drones'] = [];
    drone_list.forEach(function (drone) {
        new_drone_names.push(drone.drone_name);
        drone_names_and_ids.push({ drone_name: drone.drone_name, pk: drone.pk });
        drone.video_frame_url = NO_ACTIVE_LIVE_STREAM_SESSION_ERROR_MESSAGE;
        drone.detected_frame_url = NO_ACTIVE_DETECTION_SESSION_ERROR_MESSAGE;
        drone.detected_frame = [];
        drone.detection = { detection_status: DETECTION_DISCONNECTED };
        webSocketMessage['drones'].push(drone);
    });
    ...
    add_list_items_to_trajectories_drone(new_drone_names);
    ...
    add_list_items_to_map_tools_drones(new_drone_names);
    add_list_items_to_enable_detections_list(new_drone_names);
    add_list_items_to_uav_missions_lidar(new_drone_names);
    add_list_items_to_uav_missions_water_sampler(new_drone_names);
...
function updateDroneObjectOnMap(telemetry, DroneObject) {
    try {
        droneObj = {};
        ...
        displayDroneData(droneObj, DroneObject.droneID);
        droneObj = {};
    } catch (error) {}
    DroneObject.droneInfo.currentBatteryLevel = telemetry.battery_percentage;
    if (DroneObject.droneObject !== undefined && DroneObject.droneObject.selected) {
        updateMissionButtonStates(DroneObject.droneObject);
        updateBuildMapButtonStates(DroneObject, true);
    }
    if (DroneObject.droneInfo.currentCoordinate !== null) {
        DroneObject.droneInfo.previousCoordinate = DroneObject.droneInfo.currentCoordinate; //Previous and current location needed for the line layer
    }

    DroneObject.droneInfo.currentCoordinate = [telemetry.lon, telemetry.lat, telemetry.alt];
    DroneObject.droneInfo.heading = telemetry.heading;
    DroneObject.droneInfo.velocity = telemetry.velocity;
    DroneObject.droneInfo.altitude = telemetry.alt;

    if (DroneObject.droneObject !== undefined) {
        if (temp_drones !== []) {
            addTooltipOnDrones(temp_drones);
            temp_drones = [];
        }
        DroneObject.droneObject.setCoords([
            DroneObject.droneInfo.currentCoordinate[0],
            DroneObject.droneInfo.currentCoordinate[1],
            DroneObject.droneInfo.currentCoordinate[2],
        ]);
        DroneObject.droneObject.setRotation(-telemetry.heading);
        if (isFirstTimeLoad && telemetry.lon !== 0 && telemetry.lat !== 0) {
            isFirstTimeLoad = false;
            map.flyTo({ center: DroneObject.droneInfo.currentCoordinate, zoom: 20 });
            remove_line_layers_from_map_drone(get_all_drone_info_array());
        }
    }
}

function createDronesTrajectoryLineOfReplay(time_series_data) {
    time_series_data
        .filter((singleDictionaryValue) => singleDictionaryValue.type == 'droneTelemetry')
        .forEach((singleDroneTelemetryValue) => {
            if (singleDroneTelemetryValue.lat != 0 && singleDroneTelemetryValue.lon != 0) {
                droneObject = get_all_drone_info_array().find((droneObject) => droneObject.droneID === singleDroneTelemetryValue.drone);
                droneObject.droneInfo.currentCoordinate = [
                    singleDroneTelemetryValue.lon,
                    singleDroneTelemetryValue.lat,
                    singleDroneTelemetryValue.alt,
                ];
                updateDroneLineLayer(droneObject, {
                    source: droneObject.droneInfo.previousCoordinate,
                    dest: droneObject.droneInfo.currentCoordinate,
                    color: [23, 184, 190],
                });
                droneObject.droneInfo.previousCoordinate = droneObject.droneInfo.currentCoordinate;
            }
        });
}
function checkIfDronesHaveWeather(time_series_data) {
    const filteredWeatherDrones = time_series_data.filter(
		(singleDictionaryValue) => {
			return singleDictionaryValue.type === "weatherDrone";
		}
	);

	const uniqueDroneNames = new Set();
	filteredWeatherDrones.forEach((dictionary) => {
		if (dictionary.drone) {
			uniqueDroneNames.add(dictionary.drone);
		}
	});
	add_list_items_to_map_tools_weather(Array.from(uniqueDroneNames));
}