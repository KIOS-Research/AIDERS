 ...
 **/
{
    ...
    function updateImageMarkers(images) {
        images.forEach(function (image) {
            // console.log(image);
            if (!(image.session_id in allDeviceImages)) {
                allDeviceImages[image.session_id] = {};
            }
            if (allDeviceImages[image.session_id][image.id] === undefined || allDeviceImages[image.session_id][image.id] === '') {
                // create the popup
                path = dutils.urls.resolve('media', { path: image['path'] });
                var popup = new maplibregl.Popup({ offset: 10, maxWidth: 'inherit' }).setHTML(
                    "<img width='300' height='auto' style='padding:5px;' src='" + path + "'></img>"
                );
                // create a DOM element for the marker
                var el = document.createElement('div');
                el.className = 'marker';
                el.style.backgroundImage = 'url(' + dutils.urls.resolve('device_images_marker') + ')';
                el.style.backgroundSize = '50px 50px';
                el.style.width = '50px';
                el.style.height = '50px';

                // add marker to map
                marker = new maplibregl.Marker(el).setLngLat([image.longitude, image.latitude]).setPopup(popup).addTo(map);
                allDeviceImages[image.session_id][image.id] = marker;
            }
        });
    }
