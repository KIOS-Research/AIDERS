{
        ...
        if (pressed) {
            if (droneIndex > -1) {
                ...
                create_detection_confirmation_dialog(DETECTION_TYPES, message, title).then(function (detectorType) {
                    ...
                    postElementId('Show Detection Objects', pressed);
                });
            ...
            postElementId('Detection object ' + toggleID, pressed);
    ...
    function visualize_detected_objects(detected_cars, drone) {
        ...
        webSocketMessage['drones'].find((droneObject) => droneObject.drone_name === drone)['detected_frame'] = [];
    }
    ...
    function update_drone_detection_state_on_api(droneID, detection_status, detectorType = 'None') {
        ...
        if (detectorType === 'None') {
            ...
            data = {
                detection: { drone_name: droneID, operation_name: CURRENT_OP, detection_status: detection_status, detection_type: detectorType },
            };
        }
            ...
            webSocketMessage['drones'].forEach(function (drone) {
                if (drone['drone_name'] == droneID) {
                    ...
                    if (drone['detected_frame_url'] === NO_ACTIVE_DETECTION_SESSION_ERROR_MESSAGE) {
                        ...
                        document.getElementById('det-img-' + droneID).src = dutils.urls.resolve('media', { path: latest_frame_path });
                    }
                    if (playing) {
