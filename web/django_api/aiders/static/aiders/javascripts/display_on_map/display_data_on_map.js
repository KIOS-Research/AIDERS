 ...
 * */
{
    ...
    function display3DObjectResults() {
        ...
        let selected_orthophotos = [];
        JSON.parse(sessionStorage.getItem(SessionProfileKeys.SELECTED_ALGORITHMS)).forEach(function (algorithm) {
            if (algorithm['fields']['algorithm_name'] === 'CREATE_3D_OBJECT_ALGORITHM') {
                selected_3d_objects.push(algorithm);
            }
            if (algorithm['fields']['algorithm_name'] === 'CREATE_ORTHOPHOTO_ALGORITHM') {
                selected_orthophotos.push(algorithm);
            }            
        });
        if (selected_3d_objects.length === 0 && selected_orthophotos.length === 0) return;

        create_popup(WARNING_ALERT, 'Loading 3D Objects...', 'loading3DObj');

        selected_3d_objects.forEach((obj) => {
            load3DObject(obj.fields.output.object_url, obj.fields.output.lower_right_coord);
        });
        
        selected_orthophotos.forEach((obj) => {
            loadOrthophoto(obj.fields.output.orthophoto_url, obj.fields.output.upper_left_coord, obj.fields.output.upper_right_coord, obj.fields.output.lower_right_coord, obj.fields.output.lower_left_coord);
        });        
    }
}
