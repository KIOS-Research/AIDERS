	...
	function requestLidarMeshDataForVisualization(_sessionId) {
		...
		$.ajax(settings).done(function (_response) {
			loadLidarMeshOnMap(
				_sessionId,
				_response.data.latitude,
				_response.data.longitude,
				_response.data.altitude,
				_response.data.path
			);
		});
	}
	function loadLidarMeshOnMap(_sessionId, _latitude, _longitude, _altitude, _filePath) {
		let mesh3dObject;
			...
			map.addLayer({
				...
				onAdd: function (map, mbxContext) {
					...
					tb.loadObj(options, function (model) {
						mesh3dObject = model.setCoords([_longitude, _latitude, _altitude]);
						tb.add(mesh3dObject);
					});
				},
