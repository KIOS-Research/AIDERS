...
async def scan_for_drone_remote_id_continuous(scan_duration=10):
    """
    Continuously scan for Remote ID devices and collect advertisement data.
    
    Args:
        scan_duration (int): Duration to scan in seconds
    
    Returns:
        dict: Dictionary mapping device addresses to their collected data
    """
...
async def main():
    ...
    remote_id_devices = await scan_for_drone_remote_id(
        scan_duration=60,
        collect_messages=True
    )
    
    print(f"\n{'='*60}")
    print(f"Scan complete. Found {len(remote_id_devices)} Remote ID device(s).")
    print(f"{'='*60}")
    
    if not remote_id_devices:
        print("No drone Remote ID devices detected.")
    else:
        # Save results to JSON file
        output_filename = f"remote_id_scan_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        output_data = {
            "scan_timestamp": datetime.now().isoformat(),
            "total_devices": len(remote_id_devices),
            "devices": remote_id_devices
        }
        ...
        # Print summary
        total_messages = sum(len(device.get("messages", [])) for device in remote_id_devices)
